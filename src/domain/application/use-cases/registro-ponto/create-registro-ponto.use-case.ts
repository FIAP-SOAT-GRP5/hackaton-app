...
export class CriarRegistroPontoUseCase implements ICriarRegistroPontoUseCase {
	...
	private async buscarRegistroPontoOuCriar(
		usuario: CurrentUser,
		data: Date
	...
	async registrar(usuario: CurrentUser): Promise<RegistroPonto> {
		...
		const registroPonto = await this.buscarRegistroPontoOuCriar(
			usuario,
			agora.toJSDate()
		);
	...
	// export class CreateRegistroPontoUseCase implements ICreateRegistroPontoUseCase {
	// 	constructor(private readonly repository: IRegistroPontoRepository) {}
